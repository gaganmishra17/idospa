...
@interface UpComingBoolingList ()
@property (strong, nonatomic) CCKFNavDrawer *rootNav;

@end

@implementation UpComingBoolingList
...
-(void)GetBookingHistoryList
{
    ...
    {
        NSURL *url = [NSURL URLWithString:[[NSString alloc] initWithFormat:@"%@%@%@",apiBase,apiUpComingBooking,[[appConfig sharedInstance].DIC_UserDetails valueForKey:@"accesstoken"]]];
        
        //NSURL *url = [NSURL URLWithString:[[NSString alloc] initWithFormat:@"http://ids2.1s.my/?api=1&task=upcomingbooking&accesstoken=5e84ab8c86cd67eb63fa43e44"]]; //Test-URL
...
-(void)BookingListRespose:(ASIHTTPRequest *)request
{
    ...
    if (parseDict != NULL)
    {
        ...
        else
        {
            ...
            //NSLog(@"Data : %@",Str_dataCheck);
            if ([Str_dataCheck isEqualToString:@""])
            {
                //NSLog(@"No Data");
                [[appConfig sharedInstance] FailAlertCall:@"oops !" Message:@"You have no upcoming booking.." ViewContriller:self];
            }
            else
            {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    LBL_Name.text = [NSString stringWithFormat:@"#%@",[ARY_Data valueForKey:@"booking_id"][indexPath.row]];
    
    UILabel *LBL_Location = (UILabel *)[cell viewWithTag:3];
    LBL_Location.text = [NSString stringWithFormat:@"%@",[ARY_Data valueForKey:@"item_name"][indexPath.row]];
    ...
    AsyncImageView *IMG_QR = (AsyncImageView *)[cell viewWithTag:6];
    IMG_QR.imageURL = [NSURL URLWithString:[NSString stringWithFormat:@"%@",[ARY_Data valueForKey:@"qr_code_image"][indexPath.row]]];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    UpComingBookingDetails *viewControler = [[UIStoryboard storyboardWithName:@"Main" bundle:nil] instantiateViewControllerWithIdentifier:vcUpComingBookingDetails];
